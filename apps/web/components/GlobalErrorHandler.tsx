  ...
  window.addEventListener(
    ...
    (event: PromiseRejectionEvent) => {
      ...
      if (isChunkLoadError) {
        ...
        const lastReload = sessionStorage.getItem('web_chunk_reload_ts');
        const now = Date.now();
        if (!lastReload || now - parseInt(lastReload, 10) > 10000) {
          sessionStorage.setItem('web_chunk_reload_ts', now.toString());
      ...
      if (
        ...
        event.stopImmediatePropagation();
        event.preventDefault();
        console.warn('AxumMarket captured unhandled Event rejection:', {
